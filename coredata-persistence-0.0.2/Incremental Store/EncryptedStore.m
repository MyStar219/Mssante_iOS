                                           ...
                                           :(NSString*)dbName
{
    ...
    NSDictionary *pragmaOptions = @{ @"synchronous": @"FULL" };
    
    NSDictionary *options = @{
                              EncryptedStorePassphraseKey : passcode,
    ...
    NSURL *storeUrl = [NSURL fileURLWithPath: [basePath stringByAppendingPathComponent: dbName]];
    ...
    if (error) {
        NSLog(@"EncryptedStore - Unable to add persistent store : %@, \nerror : %@", store, error);
        return nil;
    }
        ...
        BOOL success = [self performInTransaction:^{
            ...
            if (hasTable) {
                ...
                if (statement != NULL && sqlite3_step(statement) == SQLITE_ROW) {
                    ...
                    NSLog(@"EncryptedStore - loadMetaData - error : %@",*error);
                    sqlite3_finalize(statement);
                    return NO;
                }
                sqlite3_finalize(statement);
                ...
                if ([[options objectForKey:NSMigratePersistentStoresAutomaticallyOption] boolValue] &&
                    ...
                    if (oldModel && newModel && ![oldModel isEqual:newModel]) {
                        ...
                        if (![self migrateFromModel:oldModel toModel:newModel error:error]) {
                            NSLog(@"EncryptedStore - loadMetaData - migration fail");
                            return NO;
                        }
                        ...
                        if (![self saveMetadata]) {
                            ...
                            NSLog(@"EncryptedStore - loadMetadata - save error : %@", *error);
                            return NO;
                        }
...
- (BOOL)configureDatabasePassphrase {
    ...
    if(cacheSize != nil){
        ...
        if (statement == NULL || sqlite3_finalize(statement) != SQLITE_OK){
            ...
            NSLog(@"EncryptedStore - Error: statement is NULL or could not be finalized");
            return NO;
        } else {
            ...
            if(actualCacheSize == [cacheSize intValue]) {
                // succeeded
                NSLog(@"EncryptedStore - Cache size successfully set to %d", actualCacheSize);
            } else {
                // failed...
                NSLog(@"EncryptedStore - Error: cache size set to %d, not %d", actualCacheSize, [cacheSize intValue]);
                return NO;
            }
        }
    }
...
- (BOOL)resetDatabasePassphrase:(NSString*)passphrase {
    //    NSString *passphrase = [[self options] objectForKey:EncryptedStorePassphraseKey];
    if (sqlite3_open([[[self URL] path] UTF8String], &database) == SQLITE_OK && passphrase) {
        const char *string = [passphrase UTF8String];
        int status = sqlite3_rekey(database, string, strlen(string));
        NSLog(@"EncryptedStore - resetting the database key %d",status);
        string = NULL;
        passphrase = nil;
        return (status == SQLITE_OK);
    }
...
- (sqlite3_stmt *)preparedStatementForQuery:(NSString *)query {
    ...
    if (debug)
    {NSLog(@"EncryptedStore - SQL DEBUG: %@", query); }
    sqlite3_stmt *statement = NULL;
    if (sqlite3_prepare_v2(database, [query UTF8String], -1, &statement, NULL) == SQLITE_OK) { return statement; }
    if(debug) {NSLog(@"EncryptedStore - Could not prepare statement: %s\n", sqlite3_errmsg(database));}
    return NULL;
}
...
- (void)parseExpression:(NSExpression *)expression
    ...
    // unsupported type
        ...
        NSLog(@"EncryptedStore - %s Unsupported expression type %ld", __PRETTY_FUNCTION__, (unsigned long)type);
    }
}
