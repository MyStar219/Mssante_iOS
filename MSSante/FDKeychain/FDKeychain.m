	...
	inAccessGroup: (NSString *)accessGroup
{
	...
	else
	{
		...
		if (FDIsEmpty(itemFromKeychain) == YES)
		{
            ...
            DLog(@"If the item does not exist, add it to the keychain %ld",result);
			
			if (result != noErr)
			{
				DLog(@"Could not add keychain item. (Error code: %ld)", 
					result);
			}
		}
		...
		else
		{
            ...
            DLog(@"If the item does exist, update the item in the keychain. %ld",result);
			
			if (result != noErr)
			{
				DLog(@"Could not update keychain item. (Error code: %ld)", 
					result);
			}
		}
	}
	...
	inAccessGroup: (NSString *)accessGroup
{
	...
	if (result != noErr)
	{
		...
		{
			DLog(@"Could not delete item with key '%@' from the keychain because it does not exist.", 
				key);
		}
		else
		{
			DLog(@"Could not delete keychain item. (Error code: %ld)", 
				result);
		}
	}
}
