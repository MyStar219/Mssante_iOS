...
+ (NSString *)storeUUID:(BOOL)itemExists
{
	...
	if (status != noErr)
	{
		DLog(@"BPXLUUIDHandler Keychain Save Error: %ld", status);
		uuid = nil;
	}
...
+ (NSString *)UUID
{
	...
	if (status != noErr) 
	{
		...
		{
			DLog(@"BPXLUUIDHandler Unhandled Keychain Error %ld", status);
			return nil;
		}
	}
...
+ (void)reset
{
	...
	if (status != noErr)
	{
		DLog(@"BPXLUUIDHandler Keychain Delete Error: %ld", status);
	}
	CFRelease(query);
}
