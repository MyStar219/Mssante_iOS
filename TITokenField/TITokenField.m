...
//    CGFloat availableWidth = self.bounds.size.width - self.leftView.bounds.size.width - self.rightView.bounds.size.width;
    ...
    if (_numberOfLines>1 && _tokens.count>1){
        
		[_tokens enumerateObjectsUsingBlock:^(TIToken * token, NSUInteger idx, BOOL *stop){[token removeFromSuperview];}];
            untokenized = [NSString stringWithFormat:@"%d destinataires", titles.count];
...
- (void)removeToken:(TIToken *)token {
	...
	if (shouldRemove){
		...
		[[NSNotificationCenter defaultCenter] postNotificationName:@"didUpdateTokens" object:self];
		[self showOrHidePlaceHolderLabel];
...
/* show tokens in the textfield*/
- (void)showTokens:(NSString*)tokenString {
	if (![tokenString isEqualToString:kTextEmpty]){
		self.text = tokenString;
...
- (void)textFieldDidBeginEditing:(UITextField *)textField {
    DLog(@" TITOKENFIELD textFieldDidBeginEditing");
...
NSLineBreakMode const lLineBreakMode = NSLineBreakByTruncatingTail;
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    CGSize titleSize;
        ...
        paragraphStyle.lineBreakMode = lLineBreakMode;
        NSDictionary *attrDict = [NSDictionary dictionaryWithObjectsAndKeys:_font,NSFontAttributeName,paragraphStyle,NSParagraphStyleAttributeName,nil];
        CGRect labelRect = [_title boundingRectWithSize:(CGSize){_maxWidth - hTextPadding - accessoryWidth, CGFLOAT_MAX} options:NSStringDrawingUsesLineFragmentOrigin attributes:attrDict context:nil];
        titleSize = labelRect.size;
        }
...
- (void)drawRect:(CGRect)rect {
    ...
    if(IS_IOS_7_OR_EARLIER){
        ...
        paragraphStyle.lineBreakMode = lLineBreakMode;
        NSDictionary *attrDict = [NSDictionary dictionaryWithObjectsAndKeys:_font,NSFontAttributeName,paragraphStyle,NSParagraphStyleAttributeName,nil];
        [_title drawInRect:textBounds withAttributes:attrDict];
    }
